...
function getDataFromServer(){
    ...
    var reftoDiv = document.getElementById("myContainer");

    var reftoServer = new XMLHttpRequest();

    debugger;

    reftoServer.onreadystatechange = () =>{
        if(reftoServer.status == 200 && reftoServer.readyState == 4){

            // var resposnceFromXML = reftoServer.responseText;
    
            var JSString = JSON.parse(reftoServer.responseText);

            var people = JSString.data;
            ...
            for(i = 0; i < people.length; i++){
                ...
                reftoDiv.innerHTML = reftoDiv.innerHTML + content;

            }
        }
    }

    reftoServer.open("GET","https://reqres.in/api/users");
